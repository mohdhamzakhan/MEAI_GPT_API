...
namespace MEAI_GPT_API.Services
{
    ...
    public class DynamicRagService : IRAGService
    {
        ...
        private List<string> GetPolicyFiles(string plant)
        {
            ...
            // ✅ NEW: any additional folder listed in appsettings.json under
            // DynamicRAG:AdditionalDocumentSources gets picked up here
            // automatically — no code change needed to add a new source.
            // If RestrictedToPlants is set for a source, it's only scanned
            // during this plant's indexing pass when `plant` is in that list;
            // otherwise it's included for every plant (like Centralized).
            foreach (var source in _config.AdditionalDocumentSources ?? new List<DocumentSourceOptions>())
            {
                if (string.IsNullOrWhiteSpace(source.Folder))
                    continue;

                var isRestricted = source.RestrictedToPlants != null && source.RestrictedToPlants.Any();
                if (isRestricted && !source.RestrictedToPlants!.Contains(plant, StringComparer.OrdinalIgnoreCase))
                {
                    continue; // this plant isn't on the allowlist for this source
                }

                var sourcePath = Path.Combine(_chromaOptions.PolicyFolder, source.Folder);
                if (!Directory.Exists(sourcePath))
                {
                    _logger.LogWarning($"⚠️ Configured document source folder not found: {sourcePath}");
                    continue;
                }
            ...
            try
            {
                    ...
                    var context))
                {
                    ...
                    if (turn != null)
                    {
                        ...
                        _appreciatedAnswerStore.Add(turn.Question, turn.Answer, chunks);
                    }
                }
            }
            catch (Exception ex)
        ...
        public async Task LoadHistoricalAppreciatedAnswersAsync()
        {
            try
            {
                ...
                foreach (var answer in appreciatedAnswers)
                {
                    ...
                    _appreciatedAnswerStore.Add(answer.Question, answer.Answer, chunks);
                }
        ...
        private string BuildOptimizedContext(List<RelevantChunk> chunks, string plant)
        {
            ...
            // ── In BuildOptimizedContext ─────────────────────────────────────────────────
            // The filter threshold uses Similarity (raw cosine) — keep it on Similarity,
        ...
        private async Task<(string Answer, List<RelevantChunk> Chunks)?> CheckAppreciatedAnswerAsync(string question)
        {
            ...
            {
                _logger.LogWarning(ex, "CheckAppreciatedAnswerAsync failed");
                return null;
            }
        }

        /// <summary>
        /// Builds the message list including full conversation history,
        /// then streams tokens. The HttpResponseMessage is owned by a
        ...
        public async Task SaveCorrectionAsync(string question, string correctAnswer, string model)
        {
            ...
            lock (_lockObject)
            {
                ...
                if (_correctionsCache.Count >= 500)
                {
                    var trimmed = _correctionsCache.TakeLast(400).ToList();
                    while (_correctionsCache.TryTake(out _)) { }
                    foreach (var item in trimmed) _correctionsCache.Add(item);
                }
        ...
        // ✅ NEW: replaces a bare "length >= 20" filter, which let pure
        // boilerplate through — a company letterhead/address block alone
        // ("Mitsubishi Electric Automotive India Private Limited / Plot
        // Number 1059-1060, Sanand-II, Industrial Estate, Ahmedabad-382110,
        // Gujarat") easily clears 20 characters and was observed indexed as
        // a standalone chunk in practice. Every such chunk that gets
        // indexed competes for a real retrieval slot against genuinely
        // informative content — directly contributing to "only surfaced
        // 1-2 real policies" type failures when a boilerplate chunk from
        // the anchor document displaces a substantive one.
        private bool IsChunkWorthIndexing(string text)
        {
        ...
        '\t'
            ...
            // Genuinely short chunks (even if not letterhead) rarely carry
            // enough standalone context to be useful when retrieved on
            // their own — 8 words is roughly "a heading and nothing else".
            if (wordCount < 8) return false;

            // Letterhead / address-block pattern: company name + plot/address
            // details + pincode, with little else. Matched conservatively
            // (both an address-like signal AND a low overall word count) so a
            ...
            if (hasAddressSignal && hasCompanyNameOnly && wordCount < 25)
            {
                _logger.LogDebug($"⏭️ Skipping likely letterhead/boilerplate chunk ({wordCount} words): {trimmed.Substring(0, Math.Min(60, trimmed.Length))}...");
                return false;
            }

            // A chunk that's ONLY a section heading with no body content
            // (e.g. just "PURPOSE" or "1. SCOPE" and nothing else) — headings
          ...
          string plant)
        {
            try
            {
                ...
                var validChunks = chunks
                  ...
                  .Where(chunk => IsChunkWorthIndexing(chunk.Text))
                  .ToList();
        ...
        //                Text = documentText,
        //                Source = sourceFile,
        //                Similarity = rawCosineSimilarity,      // untouched cosine [0,1]
        //                PolicyType = policyType,
        //                RelevanceScore = boostedScore          // used only for ordering
        //            });
        //        }
        //    }
          ...
          string originalQuery)
        {
            ...
            for (int i = 0; i < documents.Length; i++)
            {
                ...
                // ChromaDB returns L2 distance for cosine collections after
                // normalisation, so 1 - distance is the cosine similarity.
                // We clamp to [0, 1] defensively.
                var rawSimilarity = Math.Clamp(1.0 - distances[i].GetDouble(), 0.0, 1.0);

                // Hard floor — discard anything with no meaningful signal
                if (rawSimilarity < 0.10)
                    continue;
                ...
                // then apply additive boosts.  We track the boost total separately
                // so it is easy to read and tune.
                double boost = 0.0;
                ...
                // Plant-specific content preferred over centralized
                if (lowerSource.Contains(currentPlant.ToLowerInvariant()))
                {
                    boost += 0.10;
                }
                ...
                // ✅ NEW: exact metadata match for any OTHER configured
                // reference type (Clause, Form, etc.). Annexure is handled
                // above since its metadata key also feeds the
                // annexure-link-resolution feature further down; every other
                // type configured under DynamicRAG:ReferenceTypes gets the
                // same treatment here without needing its own code block.
                foreach (var refType in _config.ReferenceTypes ?? new List<ReferenceTypeOptions>())
                {
                    ...
                    {
                        boost += 0.70;
                        _logger.LogInformation("Exact {Type} metadata match → raw={Raw:F3}", refType.Name, rawSimilarity);
                    }
                }

                // ✅ NEW: boost content when the query mentions this chunk's
                // source_tag by name (e.g. a customer name, product name, or
                // any other subfolder-derived tag under an additional
                    ...
                    var sourceTagProp))
                {
                    ...
                    if (!string.IsNullOrEmpty(sourceTag) &&
                      originalQuery.Contains(sourceTag, StringComparison.OrdinalIgnoreCase))
                    {
                        boost += 0.40;
                        _logger.LogInformation("Source tag match: {Tag} → raw={Raw:F3}", sourceTag, rawSimilarity);
                    }
                }
                ...
                var relevanceScore = rawSimilarity + boost;
        ...
        private double CalculateDynamicSectionRelevance(RelevantChunk chunk, SectionQuery sectionQuery)
        {
            ...
            if (sectionQuery.IsAnnexure)
            {
                if (IsExactReferenceMatch(lowerText, sectionQuery))
                {
                    relevance += 0.4;
                }
            }
        ...
        private bool IsSectionContentDynamic(string text, string source, SectionQuery sectionQuery)
        {
          ...
          "no"
            ...
            if (sectionQuery.IsAnnexure)
            {
                // Reference types other than "Section" don't have meaningful
                // "expected topics" the way sections do — an exact-match
                // miss means this chunk isn't the one being asked for.
                return IsExactReferenceMatch(lowerText, sectionQuery);
            }
        ...
        /// <summary>
        /// Generic exact-match check for any reference type — Annexure keeps
        /// using its own hand-tuned patterns above (multiple phrasing
        /// variants, more precise than a single config regex would be); any
        /// other type configured under DynamicRAG:ReferenceTypes re-runs its
        /// own Pattern against the text and checks for a matching number.
        /// </summary>
        private bool IsExactReferenceMatch(string lowerText, SectionQuery sectionQuery)
        {
            if (string.Equals(sectionQuery.ReferenceType, "Annexure", StringComparison.OrdinalIgnoreCase))
            {
                return IsExactAnnexureMatch(lowerText, sectionQuery.SectionNumber);
            }
        ...
        private Dictionary<string, object> CreateChunkMetadata(string sourceFile, DateTime lastModified, string modelName, string text, string plant, string sectionId = "", string title = "", string documentType = "")
        {
          ...
          },
          {
            ...
            // ✅ NEW: extract references for every reference type configured
            // under DynamicRAG:ReferenceTypes (Annexure, Clause, Form, etc.)
            // — generalizes what was previously hardcoded to Annexure only.
            // Numbered references like these are exact-lookup queries where
            // dense embedding similarity is unreliable (a short,
            // low-information query phrase competing against a large,
            // multi-topic chunk), so storing the exact numbers as metadata
            // lets retrieval fall back to a precise match instead of relying
            // only on similarity.
            var configuredRefTypes = _config.ReferenceTypes ?? new List<ReferenceTypeOptions>();

            if (configuredRefTypes.Any())
            {
                foreach (var refType in configuredRefTypes)
                {
                    if (string.IsNullOrWhiteSpace(refType.Pattern) || string.IsNullOrWhiteSpace(refType.MetadataKey))
                        continue;
                    ...
                    if (refNumbers.Any())
                    {
                        // Chroma string metadata doesn't support array types
                        // here, so store as a comma-delimited string; callers
                        // doing an exact lookup filter client-side with
                        // .Split(',').Contains(N).
                        metadata[refType.MetadataKey] = string.Join(",", refNumbers);
                    }
                }
            }
            else
            ...
            // ✅ NEW: generic, config-driven classification for any folder
            // listed in appsettings.json under AdditionalDocumentSources —
            // no hardcoded folder names. If the source is restricted to
            // specific plants, tag it with the current plant directly (it
            // then rides the existing plant-specific retrieval branch,
            // since GetPolicyFiles already only included it for allowed
                ...
                s => !string.IsNullOrWhiteSpace(s.Folder) && folderPath.Contains(s.Folder.ToLowerInvariant())) is DocumentSourceOptions matchedSource)
            {
                ...
                {
                    metadata["plant"] = "additional_source";
                    metadata["is_additional_source"] = true;
                }

                metadata["document_source"] = matchedSource.Folder;

                // Generic sub-tag from the immediate subfolder under the
                // source (e.g. Customers/AcmeCorp -> "AcmeCorp", or
                // Products/WidgetX -> "WidgetX"). Used to boost relevance
                // when that name appears in a query — works for any
                // subfolder-organized source, not just a specific concept
                // like "customer".
                var sourceIndex = sourceFile.IndexOf(matchedSource.Folder, StringComparison.OrdinalIgnoreCase);
                if (sourceIndex >= 0)
                {
            ...
            '/'
                    ...
                    {
                        metadata["source_tag"] = pathParts[0];
                    }
                }
            }
            ...
            try
            {
              ...
              "by"
                ...
                List<RelevantChunk> SelectWithGuaranteedAnchors(List<RelevantChunk> candidatePool)
                {
                    ...
                    // their slot after its own re-scoring -- without this,
        ...
        //    string question,
        //    string modelName,
        //    string sessionId,   // ← use sessionId to fetch history
        //    [EnumeratorCancellation] CancellationToken ct = default)
        //{
        //    // Pull history from the centralised service (always fresh)
        //    var history = _historyService.GetHistory(sessionId);

        //    _logger.LogInformation("Non-MEAI stream: session {S} has {N} history turns",
        //        sessionId, history.Count);

        //    // Resolve pronouns using last answer's subject
        //    var processedQuestion = question;
        //    if (history.Any() && ContainsPronoun(question))
        //    {
        //        var subject = ExtractMainSubjectFromAnswer(history.Last().Answer);
        //        if (!string.IsNullOrEmpty(subject))
        //        {
        //            processedQuestion = RewriteQuestionWithSubject(question, subject);
        //            _logger.LogInformation("Rewrote '{Original}' → '{Rewritten}'",
        ...
        //        "For simple factual questions provide a concise direct answer. " +
          ...
          default)
        {
            ...
            if (history.Any() && ContainsPronoun(question))
            {
                ...
                {
                    processedQuestion = RewriteQuestionWithSubject(question, subject);
                    _logger.LogInformation("Pronoun resolved: '{Orig}' → '{New}'", question, processedQuestion);
                }
            }
      ...
      "Whether"
        ...
        private static bool IsCommonWord(string word)
        {
            ...
            return _commonWords.Contains(firstWord);
        }
        ...
        private async Task EnsureHistorySeededAsync(string sessionId)
        {
            ...
            var existing = _historyService.GetHistory(sessionId, limit: 1);
            ...
            {
                _logger.LogWarning(ex, "Could not seed history for session {S}", sessionId);
            }
        }
          ...
          string question)
        {
            ...
            return filtered.Any() ? filtered : chunks;
        }
          ...
          ExecutionPlan plan)
        {
            ...
            try
            {
                ...
                {
                    _logger.LogInformation(
                      "🎯 Detected {Kind} query: {Num}",
                      sectionQuery.ReferenceType,
          ...
          long processingTime)
        {
            ...
            try
            {
                ...
                var entry = new ConversationEntry
                {
                    ...
                    WasAppreciated = false,
                    TopicTag = _conversationAnalysis.DetermineTopicTag(question, answer) ?? "general",
                    GenerationModel = genModel?.Name ?? "unknown",
                    EmbeddingModel = embModel?.Name ?? "unknown",
                ...
                await storage.SaveConversationAsync(entry);
            ...
            {
                _logger.LogError(ex, "Failed to save conversation for session {SessionId}", context.SessionId);
          ...
          default)
        {
            ...
            if (isOracleEbsQuery)
                ...
                systemPrompt = await _systemPromptBuilder.BuildMeaiSystemPrompt(plant, relevantChunks, question);
            else
          ...
          CancellationToken cancellationToken)
        {
            try
            {
                ...
                // Pass null for history — SafeSetupStreamRequest will use an empty list
                var stream = StreamGenerateAsync(model, prompt, null, cancellationToken);
          ...
          default)
        {
            ...
            // Pull history from the singleton so the LLM sees previous turns
            var history = _historyService.GetHistory(context.SessionId);
          ...
          default)
        {
            ...
            while (!reader.EndOfStream && !ct.IsCancellationRequested)
            {
                ...
                if (streamResponse?.Done == true)
                {
                    // Add this block to see WHY it stopped
                    var doneReason = streamResponse.DoneReason ?? "unknown";
                    if (doneReason == "length")
                        _logger.LogWarning("⚠️ Stream truncated at token limit — increase num_ctx or reduce input size");
                    else
                        _logger.LogInformation("✅ Stream complete: {Reason}, {Tokens} tokens", doneReason, tokenCount);
                    break;
                }
            }

            _logger.LogInformation("StreamGenerateAsync complete: {Tokens} tokens", tokenCount);
        }
